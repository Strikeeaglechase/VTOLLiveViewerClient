...
class MapLoader {
	...
	public async loadHeightmapFromMission(mission: MissionInfo) {
		...
		worker.onmessage = (e: MessageEvent) => {
			...
			switch (idx) {
				case 0: chunkIdx = e.data; break;
				case 1: vertices = e.data; break;
				case 2: normals = e.data; break;
				case 3: indices = e.data; break;
				case 4: colors = e.data; break;
			}
			idx++;
			if (idx == 5) {
				...
				if (loadedCount == chunksPerSide * chunksPerSide) {
					console.log(`Chunk loading fully finished. Map load process took ${((Date.now() - start) / 1000).toFixed(2)}sec Map size: ${(bytes / 1000).toFixed(0)}kb`);
					// this.onMapLoaded();
				}
			}
		};
	...
	private createChunkMesh(chunkIdx: { x: number, y: number, pixPerChunk: number; }, vertices: ArrayBuffer, normals: ArrayBuffer, indices: ArrayBuffer, colors: ArrayBuffer) {
		...
		mesh.position.set(
			chunkIdx.x * METERS_PER_PIXEL * chunkIdx.pixPerChunk,
			0,
